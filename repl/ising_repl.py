    ...
    for j, key in enumerate(keys):
        ...
        for ns in sorted(other["num_samples"].unique()):
            ...
            ax.plot(
                ...
                grp["final_tvd"],
                "-o",
                label=labels[key].format(int(ns))
            )

        # Apply log-log scale and styling
        ax.set_xscale("log")
        ax.set_yscale("log")
        ax.grid(True, which="both", ls="--", alpha=0.3)
        ...
        if i == 0:
            ax.set_title(f"{'MTM' if j == 0 else 'IW-MTM'} vs. Baseline")
        if i == 2:
            ax.set_xlabel("Step Size")
        if j == 0:
            ax.set_ylabel(f"n={L} TVD")
...
plt.tight_layout()
fig.savefig("figures/combined_ising_comparison.png", dpi=300)
...
# Function to plot a method
def plot_method(ax, jwn, label=None, color=None):
    energies = list(zip(*[d['tvds'] for d in jwn]))
    means = [np.mean(d) for d in energies]
    xs = np.arange(1000)
    ax.plot(xs, means, label=label, color=color, linewidth=2)

# Plot for each sequence length
for i, seqlen in enumerate([4, 8, 16]):
    ax = axes[i]

    # Load data
    with open(f'dumps/ising/best_pncg.json') as f:
        baseline = json.load(f)[str(seqlen)]

    with open(f'dumps/ising/best_mtm_pncg.json') as f:
        mtm = json.load(f)[str(seqlen)]

    with open(f'dumps/ising/best_iw_mtm_pncg.json') as f:
        iw = json.load(f)[str(seqlen)]

    # Plot methods
    plot_method(ax, baseline, label="Baseline", color=colors['baseline'])
    plot_method(ax, mtm, label="Multiple-Try", color=colors['mtm'])
    plot_method(ax, iw, label="Importance-Weighted", color=colors['iw'])

    # Set labels and limits
    ax.set_ylabel('TVD', fontsize=14)
    ax.set_xlabel('Step', fontsize=14)
    ax.set_xlim(0, 1000)
    ax.set_title(f'n = {seqlen}', fontsize=16)
...
# Adjust layout
plt.tight_layout()
plt.savefig('figures/ising_comp_combined.png', dpi=300)
plt.show()
