...
for seqlen in seqlens:
    ...
    for ns in sorted(mtm_seq['num_samples'].unique()):
        ...
        plt.plot(mtm_group['alpha'], mtm_group['mean_tvd'], '-o', label=f'B={int(ns)}')
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'seqlen_{seqlen}_mtm_vs_baseline.png', dpi=300)
    ...
    for ns in sorted(iw_seq['num_samples'].unique()):
        ...
        plt.plot(iw_group['alpha'], iw_group['mean_tvd'], '-o', label=f'B={int(ns)}')
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'seqlen_{seqlen}_iw_vs_baseline.png', dpi=300)
